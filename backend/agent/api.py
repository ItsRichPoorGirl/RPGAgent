...
MAX_PROJECT_SANDBOXES = int(os.getenv("MAX_PROJECT_SANDBOXES", 20))  # 20 is the default if not set
...
async def get_or_create_project_sandbox(client, project_id: str):
    ...
    try:
        ...
        project = await client.table('projects').select('sandbox').eq('project_id', project_id).single().execute()
        
        if project.data and project.data.get('sandbox', {}).get('id'):
            logger.info(f"Found existing sandbox for project {project_id}")
            return project.data['sandbox']
            
        # If we need to create a new sandbox, first try to clean up old ones
        try:
            # Get all projects with sandboxes
            all_projects = await client.table('projects').select('project_id', 'sandbox', 'created_at').not_('sandbox', 'is', None).execute()
            
            # Sort by creation date
            sorted_projects = sorted(all_projects.data, key=lambda x: x.get('created_at', ''), reverse=True)
            
            # Keep only the N most recent projects with sandboxes
            for old_project in sorted_projects[MAX_PROJECT_SANDBOXES:]:
                if old_project.get('sandbox', {}).get('id'):
                    try:
                        ...
                        sandbox = daytona.get_current_sandbox(old_sandbox_id)
                        ...
                        logger.info(f"Successfully cleaned up sandbox {old_sandbox_id}")
        ...
        await client.table('projects').update({'sandbox': sandbox_info}).eq('project_id', project_id).execute()
        logger.info(f"Created new sandbox {sandbox.id} for project {project_id}")
