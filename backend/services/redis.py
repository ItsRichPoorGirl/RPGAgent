...
def initialize():
    ...
    # Check for REDIS_URL first (Railway might set this)
    redis_url = os.getenv('REDIS_URL')
    if redis_url:
        logger.info(f"REDIS_URL found: {redis_url}")
        try:
            # Parse REDIS_URL and create client
            client = redis.from_url(redis_url, decode_responses=True)
            logger.info(f"Created Redis client from REDIS_URL")
            return client
        except Exception as e:
            logger.error(f"Failed to create Redis client from REDIS_URL: {e}")
